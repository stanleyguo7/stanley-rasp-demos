...
- `demos/word-memory-game/`：词语记忆挑战（每轮 10 词，共 100 组）
...
为避免前端运行负担过重，IGB 项目内数据分为两类：

- 运行时数据（前端直接加载）：
  - `demos/igb-practice/assets/questions_zh_mcq.json`
- 构建原料（用于题库抽取与清洗，不是前端运行必需）：
  - `demos/igb-practice/data/raw/`
  - `demos/igb-practice/data/text/`
  - 其他批处理/中间文件

建议：部署静态站点时仅发布运行时必需文件，构建原料保留在仓库用于离线维护。

## 后续方向

- 继续扩展 IGB 题库规模与质量（目标 1000+ 可练习题）
- 增加更多家庭互动/学习类小游戏
- 为题库类 demo 增加错题本、复习模式和进阶统计（已上线基础错题模式）
